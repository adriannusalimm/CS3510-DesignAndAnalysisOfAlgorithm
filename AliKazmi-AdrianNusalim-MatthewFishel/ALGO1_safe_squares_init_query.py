...
for i in range(5):
    ...
    # randomly query half of the board
    unexplored_list = list(unexplored_set)
    random.shuffle(unexplored_list)
    for i in range(int(len(unexplored_list)*0.5)):
        unexplored_list.pop(-1)
    for square in unexplored_list:
        dug_set.add(square)
        unexplored_set.remove(square)
        if squareVal(boardState, square) == 9:
            found_bombs_set.add(square)
            stepped_on_bombs += 1
        else:
            q.put(getPriorityElem(boardState, square))
        dig_count += 1
        guess_count += 1

    # start evaluating priority queue
